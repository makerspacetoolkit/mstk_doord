...
#      config.read(os.path.join(__location__, 'doord-secrets.conf'))
      ...
      slack_enabled = config.get('slack','slack_enabled')
      slack_channel = contig.get('slack','slack_channel')
      doord_host = config.get('doord', 'host')
      doord_port = config.get('doord', 'port')
      log_level = config.get('doord', 'log_level')
      ...
      def logsearch(entrydate, entryname, statuscode, slack_channel):
         if slack_enabled:
            ...
            for dates in open(accesslogfile,encoding="latin-1"):
               ...
               if not datevalue or not namevalue:
                   ...
                   slack.chat.post_message(slack_channel, logline, as_user=False)
